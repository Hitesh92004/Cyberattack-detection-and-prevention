...
DDOS_BLOCK_DURATION = 600  # Separate block duration for DDoS (e.g., 10 minutes)

# Track requests for brute force detection
request_counts = defaultdict(list)
blocked_ips = defaultdict(float)  # IP -> time when block expires (Brute Force)

# Track requests for DDoS detection
request_tracker_ddos = defaultdict(list)  # Track requests for DDoS detection
ddos_blocked_ips = defaultdict(float)     # IP -> time when DDoS block expires (new separate blocker)
...
def detect_ddos(source_ip):
    ...
    if request_count > DDOS_THRESHOLD:
        ...
        # Block the IP for DDoS (new separate blocker)
        if source_ip not in ddos_blocked_ips or current_time > ddos_blocked_ips[source_ip]:
...
def analyze_request():
    try:
        ...
        # Check if IP is blocked due to DDoS (new separate check)
        if source_ip in ddos_blocked_ips and current_time < ddos_blocked_ips[source_ip]:
            return jsonify({
                "status": "blocked",
                "message": "Your IP is blocked due to DDoS activity."
            }), 429
...
def get_blocked_ips():
    try:
        ...
        # Combine blocked_ips, ddos_blocked_ips, and sql_blocked_ips into a single response with separate reasons
        blocked = [
...
def unblock_ip():
    try:
        ...
        if ip_to_unblock in ddos_blocked_ips and ddos_blocked_ips[ip_to_unblock] > current_time:  # New DDoS unblock
            del ddos_blocked_ips[ip_to_unblock]
            request_tracker_ddos[ip_to_unblock] = []  # Reset DDoS request tracker
...
# Temporary: Simulate blocked IPs for testing (updated to include DDoS)
current_time = time.time()
blocked_ips["192.168.1.100"] = current_time + 3600  # Blocked for 1 hour (Brute Force)
ddos_blocked_ips["10.0.0.50"] = current_time + 3600  # Blocked for 1 hour (DDoS)
